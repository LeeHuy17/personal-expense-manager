...
// 🛠️ Helper: Hide reset form (không ảnh hưởng các tab khác)
const hideResetPasswordForm = () => {
    const resetForm = document.getElementById('reset-form-container');
    if (resetForm) {
        resetForm.classList.add('hidden');
        resetForm.style.setProperty('display', 'none', 'important');
        resetForm.style.setProperty('visibility', 'hidden', 'important');
    }
    
    const statusInfo = document.getElementById('reset-status-info');
    if (statusInfo) {
        statusInfo.style.setProperty('display', 'none', 'important');
    }
};

// ✅ QUAY LẠI CỦ - Tab Đăng nhập/Đăng ký/Quên mk chuyển qua lại bình thường
export const showLoginTab = () => {
    // Ẩn form reset nếu đang show
...
// 🔑 Form Reset Password - FORM RIÊNG BIỆT (không phải tab)
export const showResetTab = () => {
    ...
    // Hiện Reset, ẩn TẤT CẢ form khác
